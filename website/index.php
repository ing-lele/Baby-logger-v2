...
if(isset($_POST['category']) && in_array($_POST['category'], ["pee", "poo", "fed"])){
	...
	$sql = "SELECT ts_start, category, TIMEDIFF(ts_end,ts_start) AS duration FROM $db_table WHERE category = '$category' AND ts_start >= CURRENT_DATE() - INTERVAL ".($days-1)." day ORDER BY ts_start DESC;";
}else{
	$sql = "SELECT ts_start, category, TIMEDIFF(ts_end,ts_start) AS duration FROM $db_table WHERE ts_start >= CURRENT_DATE() - INTERVAL ".($days-1)." day ORDER BY ts_start DESC;";
}
...
while($event = mysqli_fetch_assoc($results)){
	...
	if ($event['category'] == "fed"){
		echo "<td class='fed'>". date("d M Y H:i", strtotime($event['ts_start'])) ."</td>";
